        ...
        <SectionReveal className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {industries.map((ind) => (
            <RevealItem key={ind.name}>
              <div className="group flex flex-col items-center gap-4 rounded-3xl border border-sand-300 bg-sand-50/70 p-8 shadow-soft transition-all hover:border-wine-600/40 hover:shadow-lift">
                <span className="grid h-14 w-14 place-items-center rounded-2xl bg-wine-700 text-amber-300 shadow-glow transition-transform group-hover:scale-110">
                  {ind.icon}
                </span>
